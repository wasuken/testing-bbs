...
describe('PostDetail', () => {
  test('renders post details and comments', () => {
    ...
    expect(screen.getByText(/テスト投稿jfkldjaklfjkdalsjfkldsa/i)).toBeInTheDocument();
    expect(screen.getByText(/これはテスト投稿の内容です。/i)).toBeInTheDocument();
    expect(screen.getByText(/投稿者: 投稿者A/i)).toBeInTheDocument();
    expect(screen.getByText(/投稿日: 2024-09-04/i)).toBeInTheDocument();

    expect(screen.getByRole('heading', { name: /コメント/i })).toBeInTheDocument();
